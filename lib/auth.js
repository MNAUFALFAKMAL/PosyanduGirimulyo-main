...
export const POSYANDU_ACCOUNTS = [
  ...
  ...Array.from({ length: 6 }, (_, index) => {
    ...
    return {
      ...
      email: `plamboyan${number}@posyandu.com`,
...
export function findAccount(email, password) {
  ...
  const account = POSYANDU_ACCOUNTS.find((acc) => {
    const accEmail = acc.email.replace(/[^a-zA-Z0-9@.]/g, "");
    const testEmail = normalizedEmail.replace(/[^a-zA-Z0-9@.]/g, "");
    return accEmail === testEmail;
  });

  if (!account) return null;

  return account.passwordHashes.some((passwordHash) => verifyPasswordHash(normalizedPassword, passwordHash))
    ? account
    : null;
}
